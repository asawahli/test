...
with st.expander("Upload CSV file", expanded=True):
    uploaded = st.file_uploader("Upload a CSV file (first row should be headers)", type=["csv"], accept_multiple_files=False)
...
with st.container():
    ...
    tab1, tab2, tab3 = st.tabs(["📋 Data Preview", "📊 Summary Stats", "📈 Plots / Distributions"])
    ...
    with tab3:
        ...
        if not numeric_cols:
            ...
            col = st.selectbox("Select numeric column to visualize", numeric_cols, index=0)
            ...
            st.markdown("**Pairwise plot (first 6 numeric columns)**")
            sample_cols = numeric_cols[:6]
            if len(sample_cols) > 1:
                try:
                    fig = sns.pairplot(df[sample_cols].dropna().sample(min(500, len(df))), diag_kind="kde")
                    st.pyplot(fig)
                except Exception:
                    st.info("Pairplot couldn't be generated for this dataset (maybe too large).")
...
with st.container():
    ...
    target_col = st.selectbox("Select target column (y)", all_cols, index=len(all_cols)-1)
    feature_cols = st.multiselect("Select feature columns (X). If none selected, all numeric columns except target will be used",
                                  options=[c for c in all_cols if c != target_col],
                                  default=[c for c in df.select_dtypes(include=[np.number]).columns.tolist() if c != target_col])

    if not feature_cols:
        st.warning("No feature columns selected. Select at least one feature column to proceed.")
    ...
    if apply_split:
        ...
        st.session_state.split = {
            ...
            "test_size": test_size, "random_state": int(random_state), "feature_cols": feature_cols, "target_col": target_col
        }
        st.success(f"Split applied: train={X_train.shape[0]} rows, test={X_test.shape[0]} rows.")
...
if not st.session_state.split:
    ...
    try:
        X_train, X_test, y_train, y_test = train_test_split(df[default_features], df[default_target], test_size=0.2, random_state=42)
        st.session_state.split = {
            "X_train": X_train, "X_test": X_test, "y_train": y_train, "y_test": y_test,
            "test_size": 0.2, "random_state": 42, "feature_cols": default_features, "target_col": default_target
        }
    except Exception:
...
if not st.session_state.split:
    st.warning("No valid train/test split available. Ensure you have numeric features and a numeric target.")
    st.stop()
...
with st.container():
    ...
    model_type = st.selectbox("Choose regression algorithm",
    ...
    if model_type == "Linear Regression":
        ...
        alpha = st.number_input("alpha (regularization)", min_value=0.0, value=0.1, step=0.1)
        params["alpha"] = float(alpha)
        use_scaler = True
    ...
    model_name_input = st.text_input("Model name (used as key when storing). Default = selected algorithm",
                                    value=model_type)
    ...
    with col2:
        if st.button("Store model (save into session dictionary)"):
            if st.session_state.get("last_train") is None:
                st.warning("No model trained in this session to store. Train first, then store.")
            else:
    ...
    with col3:
        if st.button("Delete stored model"):
            key_to_delete = st.selectbox("Select stored model key to delete", options=list(st.session_state.models.keys()) or ["(none)"])
        ...
        if st.button("Download last trained model (pickle)"):
            if st.session_state.get("last_train") is None:
                ...
                try:
                    ...
                    st.download_button("Click to download .pkl", data=b, file_name=f"{st.session_state.last_train['name']}.pkl", mime="application/octet-stream")
                except Exception as e:
...
with st.container():
    ...
    if not st.session_state.models:
        st.info("No models stored yet. Train and store a model to see the summary here.")
    else:
...
st.markdown("---")
st.caption("App created with Streamlit — Upload CSV with headers to begin. Stored models persist only for the current session.")
